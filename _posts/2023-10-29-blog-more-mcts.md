...
2. SEARCH: During search, they proposed to stochastically sample actions according to $\bar{\pi}$ instead of the deterministic action selection rule $$\arg\max_{a}Q(x,a)+c∙π_θ(a \mid x)\dot\frac{\sqrt\sum_bn(x,b)}{1+n(x,a)}$$. At each node x in the tree, $\bar{π}(·)$ is computed with Q-values and total visit counts at the node based on Definition 1. We label this variant as SEARCH. 
